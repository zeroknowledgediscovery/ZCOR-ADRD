...
def ternary_encoding(input_df, 
    ...
    Args:
      input_df ([type]): Dataframe with patients' records.
      phenotype_codelists ([type]): Dictionary of {phenotype name: list of phenotype codes}.
      INFERENCE_WINDOW (int): Length of the inference window used for encoding.
    ...
    Returns:
      [type]: Dataframe with sequence encodings.
    """
    data_columns = [
            'patient_id', 
            'gender', 
        ...
        [i for i in phenotype_codelists.keys()] + \
        ['%s_weeks' % i for i in phenotype_codelists.keys()] + \
        ['%s_codes' % i for i in phenotype_codelists.keys()]
    ...
    for i, row in input_df.reset_index(drop=True).iterrows():
        ...
        for phn_name, phn_codes in phenotype_codelists.items():
        ...
        for phn_name in phenotype_codelists.keys():
